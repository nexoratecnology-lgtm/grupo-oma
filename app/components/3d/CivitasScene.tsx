...
// Componente de fondo minimalista con muy pocos bloques
const MinimalSlidingBlocks: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Clase minimalista para bloques deslizantes
    class MinimalBlock {
      x: number;
      y: number;
      width: number;
      ...
      constructor(width: number, height: number) {
        ...
        this.width = Math.random() * 200 + 100; // 100-300px
        this.height = Math.random() * 200 + 100; // 100-300px
        this.speedX = (Math.random() - 0.5) * 0.3; // Muy lento
        this.speedY = (Math.random() - 0.5) * 0.3; // Muy lento
        this.rotation = Math.random() * Math.PI * 2;
        this.rotationSpeed = (Math.random() - 0.5) * 0.005; // Rotación muy lenta
      }
      ...
      update(width: number, height: number) {
        ...
        // Si un bloque se sale de la pantalla, reubicarlo al lado opuesto
      ...
      draw(ctx: CanvasRenderingContext2D) {
        ...
        // Color gris muy sutil
        ctx.fillStyle = 'rgba(100, 100, 100, 0.03)';
    ...
    // Crear solo 5 bloques en total (muy pocos)
    const blocks: MinimalBlock[] = [];
    for (let i = 0; i < 5; i++) {
      blocks.push(new MinimalBlock(dimensions.width, dimensions.height));
    }
    ...
    const animate = (timestamp: number) => {
      ...
      // Dibujar los pocos bloques
      blocks.forEach(block => {
        block.update(dimensions.width, dimensions.height);
        block.draw(ctx);
      });
